...
module.exports = {
  ...
  entry: './src/main.ts',
  output: {
    path: path.resolve(__dirname, '../dist/worker-service'),
    filename: 'main.js',
    clean: false,
  },
  externalsPresets: { node: true },
  // The workspace is a single-package Nx repo: node_modules and package.json
  // live at `txnet-backend/`, but webpack-cli runs with cwd `worker-service/`.
  // `modulesDir` must therefore be spelled out — with the default (or with
  // `modulesFromFile`, which looks for an `worker-service/package.json` that
  ...
  cache: {
    type: 'filesystem',
    cacheDirectory: path.resolve(__dirname, '../.webpack-cache/worker-service'),
  },
