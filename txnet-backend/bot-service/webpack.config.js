...
module.exports = {
  ...
  entry: './src/main.ts',
  output: {
    path: path.resolve(__dirname, '../dist/bot-service'),
    filename: 'main.js',
    clean: false,
  },
  externalsPresets: { node: true },
  // The workspace is a single-package Nx repo: node_modules and package.json
  // live at `txnet-backend/`, but webpack-cli runs with cwd `bot-service/`.
  // `modulesDir` must therefore be spelled out — with the default (or with
  // `modulesFromFile`, which looks for an `bot-service/package.json` that
  ...
  cache: {
    type: 'filesystem',
    cacheDirectory: path.resolve(__dirname, '../.webpack-cache/bot-service'),
  },
