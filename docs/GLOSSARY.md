...
| Tenant context | The tenant a unit of work belongs to, carried ambiently for the whole request (`AsyncLocalStorage`) rather than passed as an argument. Read with `TenantContext.current()`; absent is an error, never "unscoped" (ADR-0024). | tenant-context | ResolvedTenant (the value it carries), Switch scope (a surface instance, not a brand) |
| withTenant | The Prisma extension that injects the tenant context into every query on a registered model and throws when there is none. The catalog's own name for it (20.2 layer 2). | tenant-context | runAcrossTenants (the one audited escape), RLS (the database backstop, F-1202) |
